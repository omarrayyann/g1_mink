...
if __name__ == "__main__":
    ...
    elbows = ["right_elbow_link", "left_elbow_link"]
    ...
    for elbow in elbows:
        task = mink.FrameTask(
            ...
            frame_type="body",
            position_cost=5.0,
            orientation_cost=0.0,  # Set to 0 since we only care about position
            lm_damping=1.0,
        )
    ...
    # left elbow - table, right elbow - table
    # left elbow - left thigh, right elbow - right thigh
    collision_pairs = [
        (["left_elbow_collision", "right_elbow_collision"], ["table"]),
        (["left_elbow_collision"], ["left_thigh"]),
        (["right_elbow_collision"], ["right_thigh"]),
    ]
    ...
    with mujoco.viewer.launch_passive(
        ...
        # Initialize mocap bodies at their respective sites/bodies.
        for elbow, hand, foot in zip(elbows, hands, feet):
            mink.move_mocap_to_frame(model, data, f"{foot}_target", foot, "site")
            mink.move_mocap_to_frame(model, data, f"{elbow}_target", elbow, "body")
        ...
        while viewer.is_running():
            ...
            for i, (elbow_task, hand_task, foot_task) in enumerate(zip(elbow_tasks, hand_tasks, feet_tasks)):
